...
export default function AirportRoutes() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (origin && destination) setRoutes(dummyRoutes);
  ...
  return (
    <div className="flex justify-center border border-gray-200 rounded-xl p-4 mb-3 bg-white shadow hover:shadow-md transition w-full max-w-3xl">
      <p className="text-gray-800 text-base flex items-center gap-2">
        ...
        •{" "}
        <span className="font-medium">Cost = {cost}</span> • Layovers:{" "}
        {layovers.length > 0 ? (
          <span>[{layovers.join(", ")}]</span>
        ) : (
          <span>Direct</span>
        )}
      </p>
